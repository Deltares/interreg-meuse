...
for station in locs.keys():
    ...
    for n in top_n:
        ...
        plt.xticks(range(13))
        plt.title(f'Top {n} events')
        plt.xlim(0.5,12.5)
        plt.show()
        fig.savefig(os.path.join(fn_fig, f'{station}_top_hist_{n}_events.png'), dpi=400)
    ...
    for case_fig in tops.keys():
        ...
        for ax, i in zip(axs.reshape(-1), np.arange(0,np.min([len(sub_tops), 11]),1)):
            ...
            pr_fn = os.path.join(Folder_start, 'data', 'racmo', 'members_bias_corrected', 'c_wflow', 'daily', folder, f'ds_merged_{year}.nc')
 
            if sub_tops.loc[i, 'time'].year - 1 < 1950:
                pr = xr.open_dataset(pr_fn)
            else:
                pr_fn_bef = os.path.join(Folder_start, 'data', 'racmo', 'members_bias_corrected', 'c_wflow', 'daily', folder, f'ds_merged_{year_bef}.nc')
            ...
            ax.set_ylabel('')
            ax.set_xlabel('')
            ax.set_title(sub_tops.loc[i, 'time'].month)
        cax = plt.axes([0.90, 0.1, 0.01, 0.8])
        plt.colorbar(p, cax = cax, label='cum. precip. [mm]', extend='max')
        plt.show()
        fig.savefig(os.path.join(fn_fig, f'{station}_top_events_{case_fig}.png'), dpi=400)

        #Calculating normalized plots
        seas_norm = seas / seas.max(dim=('lat','lon'))
        
        seas_norm.plot(x="lon", y="lat", col="time", col_wrap=5, cmap='Reds', vmin = 0, vmax = 1)
        plt.savefig(os.path.join(fn_fig, f'{station}_top_events_{case_fig}_normalized.png'), dpi=400)
        plt.show()

        fig = plt.figure()
        seas_norm.mean(dim='time').plot(cmap='Reds', vmin = 0, vmax = 1)
        plt.show()
        fig.savefig(os.path.join(fn_fig, f'{station}_top_events_{case_fig}_normalized_mean.png'), dpi=400)
